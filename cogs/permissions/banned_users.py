...
class BannedUsers:
    ...
    async def load_banned_ids_from_file(self):
    ...
    async def add_banned_user_id(self, user_id: int, ctx):
        ...
        banned_user_ids = await self.load_banned_ids_from_file()
        if user_id not in banned_user_ids:
            banned_user_ids.append(user_id)
            await self.save_banned_ids_to_file(banned_user_ids)
    ...
    async def remove_banned_user_id(self, user_id: int, ctx):
