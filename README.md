...
- **/russianroulette** (*Russian Roulette simulator.*)
...
- **/leaderboard [category]** *(Coming Soon)* (*Shows the top users in server or bot engagement.*)

### Economy Cog Commands
- **/balance [user]** (*Check your or another user's USh balance.*)
- **/pay [user] [amount]** (*Send USh to another user.*)
- **/gamble [amount]** *(Coming Soon)* (*Gamble your USh.*)

### Reaction Roles Cog Commands
- **/create_rr_message [title] [description] [role] [emoji]** *(Coming Soon)* (*Creates a new message that users can react to for a role.*)
- **/link_rr [message_id] [role] [emoji]** *(Coming Soon)* (*Links a reaction role to an existing message.*)

### Music Cog Commands
- **/play [query]** *(Coming Soon)* (*Plays a song from a link or search query.*)
- **/skip** *(Coming Soon)* (*Skips the currently playing song.*)
- **/queue** *(Coming Soon)* (*Shows the current music queue.*)
- **/stop** *(Coming Soon)* (*Stops the music and clears the queue.*)
- **/effects [effect]** *(Coming Soon)* (*Applies an audio effect to the music.*)
